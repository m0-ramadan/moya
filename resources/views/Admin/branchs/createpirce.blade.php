...
<div class="card">
  ...
  <form class="form theme-form" action="{{route('admin.branch.pricestore')}}" method="post" enctype="multipart/form-data">
    @csrf
  <div class="row">
                        ...
                        <label class="mr-sm-2" for="validationCustom02" style="font-family: 'Cairo', sans-serif;">الى مدينة</label>
                           <div class="input-group mb-3 ">
                                    <select class="form-select" aria-label="Default select example" id="country-select"  name="city_id">
                                        <option selected>من فضلك حدد المدينة </option>
                                       @foreach($regions as $region)
                                            <option   value="{{$region->id}}"  >{{$region->region_ar}}</option>
                                         @endforeach
                                    </select>
                             </div>
                    </div>
