...
<div class="col-md-2">
    <label class="mr-sm-2" for="role" style="font-family: 'Cairo', sans-serif;">الصلاحيات  </label>
    <select class="form-control @error('role') is-invalid fparsley-error parsley-error @enderror" name="role" id="role">
    <option value="0" {{ old('role', isset($transfer) ? $transfer->role : null) == 0 ? 'selected' : '' }}>لا يملك صلاحية</option>
    <option value="1" {{ old('role', isset($transfer) ? $transfer->role : null) == 1 ? 'selected' : '' }}>تعديل وحذف </option>
</select>
    @error('branch_id')
    <span class="invalid-feedback text-black font-weight-bold text-capitalize mt-2" role="alert">
        <p>{{ $message }}</p>
